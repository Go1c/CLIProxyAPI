...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	if got := int(codexClientTestPriority(custom["priority"])); got <= 100 {
		t.Fatalf("custom priority = %v, want a non-template priority after the template range", custom["priority"])
	}
